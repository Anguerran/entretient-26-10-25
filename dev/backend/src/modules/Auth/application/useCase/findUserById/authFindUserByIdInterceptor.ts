...
import { AuthFindUserByIdCommand } from "./authFindUserByIdCommand";

export class AuthFIndUserByIdInterceptor {
  error = new AppError({
    data: ["Error while finding user at auth module/findUserById"],
    message: "error while finding user at authentication",
    status: HttpStatusEnum.S500,
  });
  constructor(private repository: AuthGateway) {}

  async execute(command: AuthFindUserByIdCommand) {
    try {
      ...
      const res = await this.repository.authFindUserById(command);
      return res;
    } catch (error) {
